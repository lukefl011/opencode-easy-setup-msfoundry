---
description: Use as planning orchestrator; delegate task decomposition, dependency mapping, validation planning, and plan-state writing (required) before review hardening, and optionally hand off to pri-build when user approves implementation
mode: primary
...
- Delegate in order: `sub-lg-task-lister` -> `sub-lg-dependency-mapper` -> `sub-lg-validation-planner` -> `sub-lg-plan-state-writer` -> `sub-lg-plan-reviewer`.
